...
function TournamentCard({
  ...
  t: TournamentOutstanding;
  admin: boolean;
  grounds: GroundOption[];
}) {
  const progress = t.totalFee > 0 ? Math.min((t.feePaid / t.totalFee) * 100, 100) : 100;
  return (
    <div className="card overflow-hidden">
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="truncate text-[17px] font-semibold tracking-[-0.01em]">{t.name}</h3>
            ...
            {t.grounds.length > 0 ? (
              <p className="mt-1 flex flex-wrap gap-1">
                {t.grounds.map((g) => (
                  <span key={g.id} className="badge bg-ios-green/10 text-[#248A3D]">
                    🏟 {g.name}
                  </span>
                ))}
              </p>
        ...
        <div className="mt-3.5">
          <div className="mb-1.5 flex items-baseline justify-between gap-3 text-[13px]">
            ...
            <span
              className={`font-semibold ${t.outstanding > 0 ? "text-ios-orange" : "text-ios-green"}`}
            >
              {t.outstanding > 0 ? `${formatMoney(t.outstanding)} outstanding` : "Fully paid"}
            </span>
          </div>
...
export default async function TournamentsPage() {
  ...
  return (
    <div className="space-y-7">
      ...
      {tournaments.length === 0 ? (
        <Section title="All tournaments · 0">
          <EmptyState
            icon="🏆"
        ...
        <>
          <Section title={`Active tournaments · ${active.length}`}>
            {active.length === 0 ? (
              <EmptyState
                icon="🏆"
                title="No active tournaments"
                description="Every tournament is complete — see the list below."
              />
            ) : (
              <div className="space-y-3">
                {active.map((t) => (
                  <TournamentCard key={t.id} t={t} admin={admin} grounds={grounds} />
                ))}
              </div>
            )}
          </Section>
          ...
          {completed.length > 0 ? (
            <Disclosure title={`Completed tournaments · ${completed.length}`}>
              {completed.map((t) => (
                <TournamentCard key={t.id} t={t} admin={admin} grounds={grounds} />
              ))}
            </Disclosure>
